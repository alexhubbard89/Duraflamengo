...
def get_marketwatch(ticker):
    ## request page
    url = "https://www.marketwatch.com/investing/stock/{}/analystestimates".format(
        ticker.lower()
    )
    ...
    try:
        ## isolatge page
        page_ar = page.find(
            "div", {"class": "element element--analyst analyst-ratings"}
        )
        del page
        ...
        return pd.DataFrame(
            [[ticker, datetime.now().date(), False]],
...
def prep_marketwatch(df_list):
    ## unpack
    df = pd.concat([pd.DataFrame(x) for x in df_list]).reset_index(drop=True)

    ## for now not always
    df = df.rename(
        columns={"3M Ago": "three_m_ago", "1M Ago": "one_m_ago", "Current": "current"}
    )
...
def pipeline(collect_threshold=0.85):
    print("start session\n\n\n")
    ## start spark session
    spark = SparkSession.builder.appName("analyst-estimates").getOrCreate()
    ...
    while collect_percent < collect_threshold:
        ...
        analyst_estimate_df_list = (
            sc.parallelize(ticker_list).map(get_marketwatch).collect()
        )
        ## uppack and append
        analyst_estimate_df = prep_marketwatch(analyst_estimate_df_list)
        collected_analyst_estimate_df = collected_analyst_estimate_df.append(
            analyst_estimate_df
        ).reset_index(drop=True)
        ## calculate percent collected
        _ = collected_list.extend(
            analyst_estimate_df["ticker"].drop_duplicates().to_list()
        )
        ticker_list = list(set(all_ticker_list) - set(collected_list))
        collect_percent = len(collected_list) / len(all_ticker_list)
