...
def distribut_collection() -> bool:
    ...
    spark = (
        SparkSession
        .builder 
        .appName('stockanalysis-profile-collection') 
        .getOrCreate()
    )
...
def distribute_migration() -> bool:
    ...
    ue_df = pd.read_csv(MB_BUFFER+'/to-collect/data.csv')
    ticker_list = ue_df['ticker'].tolist()
    migrated_list = [x.split('/')[-1].split('.csv')[0] for x in glob.glob(WRITE_PATH+'/profile/*')]
    tickers_left = (
        list( 
            set(ticker_list) 
            - 
    ...
    spark = (
        SparkSession
        .builder 
        .appName('stockanalysis-profile-migration') 
        .getOrCreate()
    )
    sc = spark.sparkContext    
    ...
    collected_results_list = (
        sc
        .parallelize(tickers_left)
        .map(lambda t: migrate_profile(t))
        .collect()
    )
    sc.stop()
    spark.stop()
...
def get_SA_financials(ticker:str, fin_type:str) -> bool:
    ...
    if fin_type == 'income_statement':
        url = (
            'https://stockanalysis.com/stocks/{}/financials/'
            .format(ticker)
        )
        fin_dir = 'income'
    elif fin_type == 'balance_sheet':
        url = (
            'https://stockanalysis.com/stocks/{}/financials/balance-sheet/'
            .format(ticker)
        )
        fin_dir = 'balance-sheet'
    elif fin_type == 'cash_flow_statement':
        url = (
            'https://stockanalysis.com/stocks/{}/financials/cash-flow-statement/'
            .format(ticker)
        )
        fin_dir = 'cash-flow'
    elif fin_type == 'ratios':
        url = (
            'https://stockanalysis.com/stocks/{}/financials/ratios/'
            .format(ticker)
        )
        fin_dir = 'ratios'
    ...
    for fin_range in ['quarterly', 'annual']:
        ...
        if len(df) > 0:
            df['ticker'] = ticker
            _ = df.to_csv(BUFFER+'/{}/{}/{}.csv'.format(fin_range, fin_dir, ticker), index=False)
    return True
...
def distribute_financials_migration() -> bool:
    ...
    for fin_dir in ['income', 'balance-sheet', 'cash-flow', 'ratios']:
        for report in ['quarterly', 'annual']:
            ...
            collected_list_tmp = [x.split('/')[-1].split('.csv')[0] for x in glob.glob(buffer_tmp)]
    ...
    spark = (
        SparkSession
        .builder 
        .appName('stockanalysis-financials-migration') 
        .getOrCreate()
    )
    sc = spark.sparkContext    
    ## distribute migration
    ...
    migrateded_results_list = (
        ...
        .map(lambda t: migrate_financials(t))
        .collect()
    )
    sc.stop()
    spark.stop()
