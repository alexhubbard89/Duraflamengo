...
def benzinga_target_price(ticker, print_url=False):
    try:
        ...
        for tr in table.find("tbody").findAll("tr"):
            data = pd.DataFrame([[x.text for x in tr.findAll("td")]], columns=cols)
...
def price_target_collection(collect_threshold=0.50):
    """
    ...
    spark = SparkSession.builder.appName(
        "benzing-price-target-collection"
    ).getOrCreate()
    ...
    collected_list = [
        x.split("/")[-1].split(".csv")[0] for x in glob.glob(BUFFER_TARGET_DIR + "*")
    ]
    ticker_list = list(set(all_ticker_list) - set(collected_list))
    ...
    while collect_percent < collect_threshold:
        ...
        yesterday_price_df_list = (
            sc_tda.parallelize(ticker_list).map(benzinga_target_price).collect()
        )

        ## calculate percent collected
        collected_list = [
            x.split("/")[-1].split(".csv")[0]
            for x in glob.glob(BUFFER_TARGET_DIR + "*")
        ]
        ticker_list = list(set(all_ticker_list) - set(collected_list))
        collect_percent = len(collected_list) / len(all_ticker_list)
...
def migrate_target_buffer():
    ## start spark session
    spark = SparkSession.builder.appName("benzing-migrate-ratings").getOrCreate()
    ...
    ## format
    collected_df["analyst_rating_date"] = pd.to_datetime(
    ...
    ## format
    dl_df["analyst_rating_date"] = pd.to_datetime(dl_df["analyst_rating_date"]).apply(
        lambda r: r.date()
    )
    ...
    collected_df = collected_df.merge(
        dl_df.drop("date_collected", 1), how="left", on=join_cols
    ).fillna(False)
    collected_df = collected_df.loc[collected_df["previously_collect"] == False].drop(
        "previously_collect", 1
    )
    date_list = sorted(collected_df["analyst_rating_date"].unique())
    dl_df_subset = dl_df.loc[dl_df["analyst_rating_date"].isin(date_list)]
    del dl_df
    for date in date_list:
        tmp_dl_df = dl_df_subset.loc[dl_df_subset["analyst_rating_date"] == date].drop(
            "previously_collect", 1
        )
        tmp_df = tmp_dl_df.append(
            collected_df.loc[collected_df["analyst_rating_date"] == date]
        ).reset_index(drop=True)
        ## format bc of append
        tmp_df["analyst_rating_date"] = pd.to_datetime(
...
def get_unusual_options(date: dt.date):
    ...
    ## format url
    url = UO_URL.format(
        BENGINZA_TOKEN=Variable.get("BENGINZA_TOKEN"), DATE=date
    ).replace("\n", "")
    ## make request
    spoof = Ip_Spoofer()
    ...
    _ = pd.DataFrame(result_list).to_csv(
        UNUSUAL_OPTIONS_BUFFER + "{}.csv".format(date), index=False
    )
    return True
...
def migrate_options_buffer():
    ...
    if len(file_list) == 0:
        return False
    collected_df = pd.concat([pd.read_csv(x) for x in file_list]).reset_index(drop=True)
    if len(collected_df) == 0:
        return False
    ## formatting
    for col in ["date", "date_expiration"]:
        collected_df[col] = pd.to_datetime(collected_df[col]).apply(lambda r: r.date())
    ...
    ## start spark session
    spark = SparkSession.builder.appName(
        "benzing-migrate-unusual-options"
    ).getOrCreate()
    ## write to data lake
    ## get date from buffer file
    date = glob.glob(UNUSUAL_OPTIONS_BUFFER + "*")[0].split("/")[-1].split(".csv")[0]
...
def get_rating_changes(date: dt.datetime) -> bool:
    ...
    url = RATING_CHANGES_URL.format(
        BENGINZA_TOKEN=Variable.get("BENGINZA_TOKEN"), DATE=date_str
    ).replace("\n", "")
    ## make request
    spoof = Ip_Spoofer()
    page = Ip_Spoofer.request_page(spoof, url)
    ...
    _ = pd.DataFrame(result_list).to_csv(RATING_CHANGES_BUFFER + file_name, index=False)
    return True
...
def migrate_rating_changes() -> bool:
    ...
    if len(collected_df) == 0:
        return False
    ## formatting
    collected_df["date"] = pd.to_datetime(collected_df["date"]).apply(
        lambda r: r.date()
    )
    df_cols = collected_df.columns
    for col in TYPE_DICT:
        ...
        collected_df[col] = collected_df[col].astype(TYPE_DICT[col])
    ## start spark session
    spark = SparkSession.builder.appName(DL_RATING_CHANGES_DIR).getOrCreate()
    ## write to data lake
    ## get date from buffer file
    date = glob.glob(RATING_CHANGES_BUFFER + "*")[0].split("/")[-1].split(".csv")[0]
    _ = utils.write_spark(spark, collected_df, DL_RATING_CHANGES_DIR, date)
    return True
