...
def format_earnings(df):
    ## remove bad data
    df_cp = df.loc[df["date"].apply(check_date)].copy().reset_index(drop=True)
    ...
    for col in df_cp.columns:
        if col in ["ticker", "quarter"]:
            df_cp[col] = df_cp[col].apply(lambda r: None if r == "" else r).astype(str)
        elif col == "date":
            ## remove extra str
            df_cp[col] = df_cp[col].apply(lambda r: r.split("(")[0])
            ## convert
            df_cp[col] = pd.to_datetime(df_cp[col]).apply(lambda r: r.date())
        elif col in float_cols:
            df_cp[col] = (
                df_cp[col].apply(lambda r: np.nan if r == "" else r).astype(float)
            )

    df_cp["collection_date"] = dt.datetime.now(pytz.timezone("America/New_York")).date()
    df_clean = df_cp[["ticker", "date", "quarter", "collection_date"] + float_cols]
...
def request_earnings(ticker: str) -> bool:
    ...
    for exchange in ["NASDAQ", "NYSE"]:
        ...
        try:
            ## get columns
            columns = []
            thead = page_table.find("thead").find("tr")
            for th in thead.findAll("th"):
                col_val = th.text.lower().replace("/", "_").replace(" ", "_")
                _ = columns.append(col_val)
            ## get data
            data = []
            tbody = page_table.find("tbody")
        ...
        if len(df) > 0:
            ...
            ## write to buffer
            _ = formated_df.to_csv(
                BUFFER + "/collected/{}.csv".format(ticker), index=False
            )
            ## if data exists exit
            return True
    ## exit loop.
...
def find_upcoming(date: dt.date) -> bool:
    ...
    mb_earnings_df["date"] = pd.to_datetime(mb_earnings_df["date"]).apply(
        lambda r: r.date()
    )
    ...
    recently_reported_ticker_df = (
        mb_earnings_df.loc[
            (
                (mb_earnings_df["reported_eps"].notnull())
    ...
    recently_reported_ticker_list = recently_reported_ticker_df["ticker"].tolist()
    #########################
    ##### Find upcoming #####
    #########################
    has_projected_date_df = (
        mb_earnings_df.loc[
            (
                (mb_earnings_df["ticker"].isin(recently_reported_ticker_list))
    ...
    missing_projected_date = list(
        set(recently_reported_ticker_list) - set(has_projected_date_df["ticker"])
    )
    missing_projected_date_df = recently_reported_ticker_df.loc[
        (
            (recently_reported_ticker_df["ticker"].isin(missing_projected_date))
            & (
                recently_reported_ticker_df["date"]
                >= dt.datetime.now().date() - dt.timedelta(60)
            )
        )
    ].copy()
    ...
    upcoming_earnings_df = (
        has_projected_date_df.loc[has_projected_date_df["date"] <= fd]
...
def distributed_collection() -> bool:
    ...
    spark = SparkSession.builder.appName(
        "marketbeat-financials-collection"
    ).getOrCreate()
    ...
    collected_list = [
        x.split("/")[-1].split(".csv")[0] for x in glob.glob(BUFFER + "/collected/*")
    ]
    to_collect_list = list(set(ticker_list) - set(collected_list))
    ## collect
    migration_results_list = (
        sc.parallelize(to_collect_list).map(lambda t: request_earnings(t)).collect()
    )
    return True
...
def migrate_mb_fins(ticker: str) -> bool:
    ...
    og_df = (
        og_df_raw.loc[og_df_raw["reported_eps"].notnull()]
        ## remove future earnings rows
    )
    new_df = pd.read_csv(fn_b)
    ## bring together
    dist_cols = list(set(og_df.columns) - set("collection_date"))
    migrate_df = og_df.append(new_df).drop_duplicates(dist_cols).reset_index(drop=True)
    _ = migrate_df.to_csv(
        WRITE_PATH_ALL_EARNINGS + "/{}.csv".format(ticker), index=False
    )
    del migrate_df
    return True
...
def migrate_daily() -> bool:
    ...
    ## start spark session
    spark = SparkSession.builder.appName(
        "marketbeat-financials-migration"
    ).getOrCreate()
    sc = spark.sparkContext
    collected_list = [
        x.split("/")[-1].split(".csv")[0] for x in glob.glob(BUFFER + "/collected/*")
    ...
    ## collect
    migration_results_list = (
        sc.parallelize(collected_list).map(lambda t: migrate_mb_fins(t)).collect()
    )
    return True
...
def make_upcoming_table() -> bool:
    ...
    """
    ue_df = pd.read_csv(BUFFER + "/to-collect/data.csv")
    ue_df["date"] = pd.to_datetime(ue_df["date"]).apply(lambda r: r.date())
    ue_df_subset = (
        ue_df.loc[ue_df["date"] <= dt.datetime.today().date() + dt.timedelta(14)]
        .copy()
        .sort_values("date")
        .reset_index(drop=True)
    )
