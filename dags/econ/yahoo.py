...
def get_trending_collected():
    return [
        ...
        .split('/')[-1]
        .split('.')[0] 
        for x in glob.glob(TRENDING_WRITE_BUFFER+'*')
    ]
...
def get_yahoo_trending():
    ...
    cols = [
        ...
        .replace(' ', '_')
        .replace('/', '_')
        .lower() for x in page.find('table').find('tr')
    ]
    df = pd.DataFrame(columns=cols)
    for tr in page.find('tbody').findAll('tr'):
        df = (
            df
            .append(pd.DataFrame([[x.text.replace('%', '') for x in tr.findAll('td')]], columns=cols))
...
def migrate_trending():
    ...
    spark = (
        SparkSession
        .builder 
        .appName('migrate-yahoo-trending') 
        .getOrCreate()
    )
    try:
        ## get data to migrate
        ...
        _ = (
            utils
            .write_spark(spark, collected_trending_df, 
                         'yahoo-trending', today))
    except:
