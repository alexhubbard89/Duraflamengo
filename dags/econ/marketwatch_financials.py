...
def format_data(df):
    ...
    df["collection_date"] = dt.datetime.now(pytz.timezone("America/New_York")).date()
    return df
...
def get_financials(ticker: str, fin_type: str, freq: str) -> bool:
    ...
    if freq == "quarter":
        url = "https://www.marketwatch.com/investing/stock/{}/financials/{}/quarter".format(
            ticker.lower(), fin_type
        )
        index_col = "date"
    else:
        url = "https://www.marketwatch.com/investing/stock/{}/financials/{}".format(
            ticker.lower(), fin_type
        )
        index_col = "year"
    ...
    all_data = (
        all_data.set_index(all_data.columns[0])
        .transpose()
        .reset_index()
...
def migrate_financials(ticker: str, fin_type: str) -> bool:
    ...
    for col in date_cols:
        buffer_df[col] = pd.to_datetime(buffer_df[col]).apply(lambda r: r.date())
        dl_df[col] = pd.to_datetime(dl_df[col]).apply(lambda r: r.date())
    ## full dataset
    full_df = (
        dl_df.append(buffer_df)
        .sort_values("collection_date", ascending=False)
        .drop_duplicates("date")
    )
    full_df = full_df.loc[full_df["date"].notnull()]
    ## replace data
    _ = full_df.to_csv(WRITE_PATH.format(fin_type, ticker), index=False)
    ## remove from buffer
...
def distributed_collection(
    collect_threshold: float, fin_type: str, loop_collect: int
) -> bool:
    """
    ...
    spark = SparkSession.builder.appName(
        "marketwatch-financials-collection"
    ).getOrCreate()
    ...
    collected_list = [
        x.split("/")[-1].split(".csv")[0] for x in glob.glob(TMP_BUFFER + "*")
    ]
...
def distributed_migration(fin_type: str) -> bool:
    ...
    ## start spark session
    spark = SparkSession.builder.appName(
        "marketwatch-financials-migration"
    ).getOrCreate()
    sc = spark.sparkContext
