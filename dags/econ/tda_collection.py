...
def get_historical_minute_price(ticker):
    ...
    try:
       ...
       ## write out individual dates
        ...
        _ = (
            pd.DataFrame()
            .to_csv(MINUTE_WRITE_BUFFER+'collected/{}.csv'.format(ticker))
        )
        return True
    except:
        return False
...
def minute_price_pipeline(collect_threshold=.85, loop_collect=240):
    """
    ...
    spark = (
        SparkSession
        .builder 
        .appName('minute-tda-price-collect') 
        .getOrCreate()
    )
    ...
    while collect_percent < collect_threshold:
        ...
        \tSample tickers: {}
        """
        print(_str.format(count, len(tickers_left), len(ticker_list),
...
def migrate_tda_minute():
    ...
    for date in date_list: 
        try:
            ...
            print('write data')
            _ = utils.write_spark(spark, collected_tickers_df, 
                            'tda-minute-price', date)
        except:
...
def daily_price_pipeline(collect_threshold=.85, loop_collect=240):
    """
    https://developer.tdameritrade.com/content/authentication-faq
    Q: Are requests to the Post Access Token API throttled?
        All non-order based requests by personal use non-commercial 
    ...
    while collect_percent < collect_threshold:
        ...
        """
        print(_str.format(count, len(tickers_left), len(ticker_list),
              len(collected_list), collect_percent, 
              YESTERDAY, ticker_list[:5]))
