...
def get_current():
    ...
    v = float([x for x in page.findAll('fin-streamer') 
                  if '^VIX' in x.get('data-symbol')][0].get('value'))
    ...
    if not os.path.isdir(dir_):
        ...
        todays_df = (
            pd.read_csv(dir_+'/data.csv')
            .append(pd.DataFrame([[now, v]], columns=['timestamp', 'close']),
                    ignore_index=True
                   )
        )
