...
def income_metrics() -> pd.DataFrame:
    ...
    income_df = (
        utils
        .read_many_csv(SA_INCOME+'/')
        [['datekey', 'ticker', 'revenue', 'gp', 'opinc', 'netinc']]
        .dropna()
        .reset_index(drop=True)
    )
    income_df['date'] = (
        pd.to_datetime(income_df['datekey'])
    ...
    income_df['operating_margin_sigma_delta'] = (
        income_df
        .groupby(['ticker'])
...
def balance_sheet_metrics() -> pd.DataFrame:
    ...
    balance_df = (
        utils
        .read_many_csv(SA_BS+'/')
        [['datekey', 'ticker', 'assetsc', 
          'inventory', 'liabilitiesc', 'debtc',
    ...
    balance_df['date'] = (
        pd.to_datetime(balance_df['datekey'])
        .apply(lambda r: r.date())
    )

    balance_df['acid'] = (
        (balance_df['assetsc'] - balance_df['inventory'])
        /balance_df['liabilitiesc']
    )
    balance_df['solvency'] = (
        balance_df['assetsc']
        /balance_df['liabilitiesc']
    )
    balance_df['debt_ratio'] = (
        balance_df['liabilities']
        /balance_df['assets']
    )
    return balance_df
...
def ro_metrics(income_df: pd.DataFrame, balance_df: pd.DataFrame) -> pd.DataFrame:
    ro_df = (
        balance_df[['ticker', 'date', 'assets', 'equity']]
        .merge(income_df[['ticker', 'date', 'netinc']],
               how='inner', on=['ticker', 'date']
              )
    )
...
def pipeline() -> bool:
    ...
    ## join
    health_df = (
        i_df[['ticker', 'date', 'gross_profit_margin', 'operating_margin', 
                   'proft_margin', 'operating_margin_sigma', 
                   'operating_margin_sigma_delta']]
        .merge(bs_df[['ticker', 'date', 'acid', 'solvency', 'debt_ratio']],
               how='inner', on=['ticker', 'date']
              )
        .merge(ro_df[['ticker', 'date', 'roe', 'roa']],
               how='inner', on=['ticker', 'date']
              )
    ...
    ## make stats 
    health_df['healthy_profit_margin'] = health_df['gross_profit_margin'] >= .6
    health_df['healthy_operating_margin'] = health_df['operating_margin'] < .15
    health_df['healthy_proft_margin'] = health_df['proft_margin'] >= .2
    health_df['healthy_acid'] = health_df['acid'] >= 1
    health_df['healthy_solvency'] = health_df['solvency'] >= 2
    health_df['healthy_debt_ratio'] = health_df['debt_ratio'] < .4
    health_df['healthy_roe'] = health_df['roe'] >= .15
    health_df['healthy_roa'] = health_df['roa'] >= .05
    ## deduplicate
