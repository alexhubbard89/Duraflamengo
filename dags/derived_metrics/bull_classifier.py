...
def add_bull_call_spread_flags(enriched_data):
    flag_cols = [
        "rsi_oversold",
        "macd_positive",
    ...
    rsi_threshold = 25  # RSI threshold for oversold condition
    macd_histogram_threshold = 0  # MACD histogram threshold for positive histogram

    # Define the additional filters
    volume_filter = (
        enriched_data["average_volume"] > 2 * enriched_data["average_volume"].mean()
    )
    relative_strength_filter = enriched_data["rsi"] < 35
    moving_average_crossover_filter = (
        enriched_data["close_avg_20"] > enriched_data["close_avg_50"]
    )
    price_breakout_filter = (
        enriched_data["close"] > enriched_data["resistance"].rolling(window=5).min()
    )
    divergence_filter = (
    ...
    enriched_data["rsi_oversold"] = enriched_data["rsi"] < rsi_threshold
    enriched_data["macd_positive"] = (
        enriched_data["macdHist"] > macd_histogram_threshold
    )
    enriched_data["volume_relative_strength"] = volume_filter & relative_strength_filter
    enriched_data["moving_avg_crossover"] = moving_average_crossover_filter
    enriched_data["price_breakout"] = price_breakout_filter
    ...
    enriched_data["bollinger_band_flag"] = (
        enriched_data["close"] > enriched_data["lower_band_bb"]
    ) & (enriched_data["close"] < enriched_data["middle_band_bb"])
    enriched_data["keltner_channel_flag"] = (
        enriched_data["close"] < enriched_data["lower_channel"]
    ) & (enriched_data["close"] > enriched_data["lower_band_bb"])
    ...
    enriched_data["price_below_lower_bb"] = (
        enriched_data["close"].rolling(window=5).min() < enriched_data["lower_band_bb"]
    )
...
def create_bullish_flag(close: pd.Series, window=20, threshold=0.05):
    """
    Create the class variable 'bullish_flag' based on the percentage change in price over a rolling window.
    If the percentage change exceeds the threshold, set the flag to 1 (bullish), otherwise set it to 0 (non-bullish).

    Args:
        enriched_data (pd.DataFrame): DataFrame containing the enriched data.
        window (int): Size of the rolling window.
        threshold (float): Threshold value for determining bullishness.

    Returns:
        pd.Series: Series with the 'bullish_flag' column added.
...
def train():
    ...
    Raw code found in: prototyping/marty/vertical-spreads/bull_call_spread_analysis.ipynb
    """
    do_not_run = True
    if do_not_run:
        return False
    ...
    # feature engineering
    extended_bullish_training_df = all_enriched_data.groupby("symbol").apply(
        add_bull_call_spread_flags
    )

    ## add class flags
    extended_bullish_training_df[
        "days_to_bullish"
    ] = extended_bullish_training_df.groupby("symbol")["close"].apply(
        create_bullish_flag
    )
    extended_bullish_training_df["bullish_flag"] = (
        extended_bullish_training_df["days_to_bullish"]
        .apply(lambda r: r > 0)
        .astype(int)
    )
    # Calculate the date corresponding to the number of days until bullish
    extended_bullish_training_df["resulting_date"] = np.where(
        extended_bullish_training_df["days_to_bullish"] == -1,
        pd.NaT,  # Return NaT (Not a Time) for days_to_bullish == -1
        extended_bullish_training_df["date"]
        + pd.to_timedelta(extended_bullish_training_df["days_to_bullish"], unit="D"),
    )
    # split validation
    ds_cut = dt.date(2023, 1, 1)
    extended_training = extended_bullish_training_df.loc[
        extended_bullish_training_df["date"] < ds_cut
    ]
    extended_validation = extended_bullish_training_df.loc[
        extended_bullish_training_df["date"] >= ds_cut
    ]

    # Define the parameter grid for the randomized search
    # found via RandomizeSearch
    param_grid = {
    ...
    X, y = extended_training[der_s.bull_20d_features], extended_training["bullish_flag"]
    ...
    X_val = extended_validation[der_s.bull_20d_features]
    y_val = extended_validation["bullish_flag"]
    ...
    nn_classifier = MLPClassifier(
        ...
        learning_rate="adaptive",  # Try different learning_rate options ('constant', 'adaptive', 'invscaling')
        learning_rate_init=0.001,  # Adjust the learning rate
        batch_size=16,  # Try different batch sizes ('auto', integer value)
        max_iter=415,  # Maximum number of training iterations
        random_state=42,
    )
    ...
    y_val_pred_proba = nn_classifier.predict_proba(X_val)[:, 1]
    y_test_pred = nn_classifier.predict(X_test)
    y_test_pred_proba = nn_classifier.predict_proba(X_test)[:, 1]
    ...
    feature_importances = np.abs(nn_classifier.coefs_[0])
    ...
    feature_importance_pairs = list(zip(der_s.bull_20d_features, feature_importances))
...
def daily_predict(ds: str, cutoff: float = 0.7) -> None:
    """
    ...
    Args:
        ...
        cutoff (float): The probability threshold for classifying as bullish.

    Returns:
        None
    """
    # Load the Bull Flag 20 Days Out Classifier
    with open(der_s.bull_20d_model_path, "rb") as file:
        bull_flag_20d_classifier = pickle.load(file)
    ...
    daily_predict_df_w_features["bullish_flag"] = (
        daily_predict_df_w_features["days_to_bullish"]
        .apply(lambda r: r > 0)
        .astype(int)
    )
    # Calculate the date corresponding to the number of days until bullish
    daily_predict_df_w_features["resulting_date"] = np.where(
        daily_predict_df_w_features["days_to_bullish"] == -1,
        pd.NaT,  # Return NaT (Not a Time) for days_to_bullish == -1
        daily_predict_df_w_features["date"]
        + pd.to_timedelta(daily_predict_df_w_features["days_to_bullish"], unit="D"),
    )
    ...
    ## predict
    X_daily = daily_predict_df_w_features[der_s.bull_20d_features]
    daily_predict_df_w_features[
        "bull_prob_20d"
    ] = bull_flag_20d_classifier.predict_proba(X_daily)[:, 1]
    daily_predict_df_w_features["bull_class_20d"] = np.where(
        daily_predict_df_w_features["bull_prob_20d"] >= cutoff, 1, 0
    )

    daily_predict_df_w_features.to_parquet(
        f"{der_s.bull_20d_classifier}/v0/data.parquet", index=False
    )
