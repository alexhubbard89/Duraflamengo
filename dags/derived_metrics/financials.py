...
MW_FIN_TRENDS_PATH = sm_data_lake_dir + '/marketwatch-financials/{FIN_TYPE}/{TICKER}.csv'
INCOME_BUFFER = sm_data_lake_dir+'/buffer/marketwatch-financials-trends/income/'
BALANCE_SHEET_BUFFER = sm_data_lake_dir+'/buffer/marketwatch-financials-trends/balance-sheet/'
...
def make_financial_trends(ticker, fin_type):
    ...
    try:
        ...
        ## clean
        drop_cols = (
            [x for x in df_raw.columns if 'growth' in x] 
            + ['ticker', 'date', 'collection_date']
        )
        cols = (
            df_raw
            .drop(drop_cols, 1)
        ...
        df = (
            df_raw
            .rename(columns=clean_cols_dict)
            [list(clean_cols_dict.values())
            + ['ticker', 'date', 'collection_date']]
        )
        del df_raw
    except:
    ...
    income_metric_cols = (
        df
        .drop(['ticker', 'date', 
               'release_date', 'collection_date'], 1)
        .columns
    )
    ...
    full_date_list = [x.date() for x in pd.date_range(df['date'].min(), dt.datetime.today())]
...
def prepare_all_financials(fin_type):
    ...
    buffer_path = MW_FIN_TRENDS_PATH.format(FIN_TYPE=fin_type, TICKER='split').split('split')[0]
    all_ticker_list = [x.split('/')[-1].split('.csv')[0] for x in glob.glob(buffer_path+'*')]

    ## start spark session
    spark = (
        SparkSession
        .builder 
        .appName('{}-trends'.format(fin_type))
        .getOrCreate()
    )
...
def migrate_financials(fin_type):
    ...
    use_columns = (
        column_appear_df
        .loc[column_appear_df['percent'] >= .9]
        .index
    )
    ## get data
    all_data = (
        utils.read_many_csv(buffer)
        [use_columns]
    )
    ...
    for col in ['date', 'collection_date', 'release_date']:
        all_data[col] = (
            pd.to_datetime(all_data[col])
            .apply(lambda r: r.date())
        )
    float_cols = (
        list(
            set(use_columns) 
            - set(['ticker', 'date', 'collection_date', 'release_date'])
        )
    )
    ...
    all_data = (
        all_data
        .loc[all_data['date'] >= dt.date(2019, 1, 1)]
        .copy()
        .reset_index(drop=True)
    )
    ...
    spark = (
        SparkSession
        .builder 
        .appName('migrate-{}-financials'.format(fin_type)) 
        .getOrCreate()
    )
    ## write each date
    for date in all_data['date'].unique():
        ...
        write_df = (
            all_data
            .loc[all_data['date'] == date]
            .reset_index(drop=True)
            .copy()
        )
        ## write to spark
        _ = utils.write_spark(spark, write_df, 
                       '/derived-measurements/{}-trends'.format(fin_type),
                       date)
        del write_df
