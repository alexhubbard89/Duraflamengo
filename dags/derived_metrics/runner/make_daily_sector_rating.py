...
if __name__ == "__main__":
    ...
    if os.path.isdir(f"{s.historical_rating}/{ds}/"):
        ...
        rating_industry_df = df[["symbol", "date", "rating_int"]].merge(
            company_profile_df[["symbol", "sector"]],
            how="inner",
            on="symbol",
        )
        if len(rating_industry_df) > 0:
            ## aggregate
            sector_agg = (
                rating_industry_df.groupby(["sector", "date"])["rating_int"]
                .describe()
                .reset_index()
            )
            ## type datasets
            sector_agg = utils.format_data(sector_agg, dm_s.sector_rating_types)
            ## write
            sector_agg.to_parquet(f"{dm_s.sector_rating}/{ds}.parquet", index=False)
