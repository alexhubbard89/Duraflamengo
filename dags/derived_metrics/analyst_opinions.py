...
## Map
rating_list = ['strong_buy', 'buy', 'overweight', 'hold', 'underweight', 'sell', 'strong_sell']
rating_map = {
    'buy': 'buy',
    'neutral': 'hold',
...
def make_scores():
    ...
    ao_subset['date'] = (
        pd.to_datetime(ao_subset['date'])
        .apply(lambda r: r.date())
    )
    
    fn = (
        glob
        .glob(AO_BUFFER + '/prepared/*.csv')[0]
        .split('/')[-1].split('.csv')[0]
        .split('T')[0]
        .split('-')
    )
    ...
    rating_count_df = (
        ao_subset
        .groupby(['ticker', 'clean_rating'])
        .agg({'pt_prior': 'count', 'date': 'max'})
        [['pt_prior', 'date']]
        .reset_index()
        .rename(columns={'pt_prior': 'count', 
                         'clean_rating': 'rating'})
    )
    ## add days since last rating
    rating_count_df['n_days_last_rating'] = (
        (date - rating_count_df['date'])
        .apply(lambda r: r.days)
    )
    ## pivot
    rating_pivot_df = (
        rating_count_df
        .pivot_table(index='ticker', columns=['rating'], values='count', aggfunc='sum')
        .fillna(0)
    )
    ## add missing ratings
    ...
    for col in rating_list:
        rating_pivot_df['{}_percent'.format(col)] = rating_pivot_df[col] / rating_pivot_df['total_ratings']
    ...
    rating_subset_df = (
        rating_pivot_df
        [['score', 'score_normalized', 'total_ratings']]
        .copy()
    ...
    full_ratings_df = (
        rating_subset_df
        .merge(n_days_df, how='outer', on='ticker')
    )
    # bring date
    if len(full_ratings_df) > 0:
        full_ratings_df['date'] = date
        _ = full_ratings_df.to_csv(AO_SCORES_BUFFER+'/scored/{}.csv'.format(date), index=False)
...
def migrate_score():
    ...
    date = (
        pd.to_datetime(
            glob.glob(AO_SCORES_BUFFER + '/scored/*.csv')[0]
            .split('/')[-1].split('.csv')[0]
        ).date()
    )
    date_cols = ['date']
    ...
    spark = (
        SparkSession
        .builder 
        .appName('migrate-ao-score') 
        .getOrCreate()
    )
    _ = utils.write_spark(spark, ao_scored, 
                          '/derived-measurements/ao-score', date)
    return True
...
def make_pt():
    ...
    ao_subset = (
        ao_subset
        .sort_values('date', ascending=False)
        .drop_duplicates(['ticker', 'analyst'])
        [['analyst', 'ticker', 
         'action_pt', 'action_company', 
         'date', 'name', 'pt_current']]
    )
    ...
    if len(ao_subset_agg) > 0:
        _ = (
            ao_subset_agg
            .to_csv(AO_PT_BUFFER + '/targeted/{}.csv'.format(date), index=False)
        )
    return True
...
def migrate_pt():
    ...
    date = (
        pd.to_datetime(
            glob.glob(AO_PT_BUFFER + '/targeted/*.csv')[0]
            .split('/')[-1].split('.csv')[0]
        ).date()
    )
    date_cols = ['date']
    for col in date_cols:
        ao_targeted[col] = (
            pd.to_datetime(ao_targeted[col])
            .apply(lambda r: r.date())
        )
    ## migrate
    ## start spark session
    spark = (
        SparkSession
        .builder 
        .appName('migrate-ao-price-target') 
        .getOrCreate()
    )
    _ = utils.write_spark(spark, ao_targeted, 
                          'derived-measurements/ao-price-target', date)
    return True
