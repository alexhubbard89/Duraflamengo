...
def split_test_train(date: dt.datetime, df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_cp = df.loc[df[TRAINING_FEATURES + [Y_CLASS]].isnull().sum(1) == 0].copy()
...
def load_training(date: dt.date):
    ...
    usecols = (
        [Y_CLASS]
        + TRAINING_FEATURES
        + ["ticker", "date", "future_date", "future_close", "future_avg_close"]
    )
    fp = TRAINING_DIR + "/{}.csv".format(date)
    full_training_df = pd.read_csv(fp, usecols=usecols)
    for col in ["future_date", "date"]:
        full_training_df[col] = pd.to_datetime(full_training_df[col]).apply(
            lambda r: r.date()
        )
...
def train_and_predict(date: dt.date, threshold=0.7) -> pd.DataFrame:
    """
    ...
    predict_df = pd.DataFrame(
        probs, index=y_test.index, columns=["p_not_growth", "p_growth"]
    )
    ...
    predict_df["price_change_percent"] = test_df["future_close"] / test_df["avg_price"]
    ## save
