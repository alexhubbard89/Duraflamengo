...
dir_full_list = [AVG_PV_DIR]
dir_sparse_list = [AO_SCORE_DIR, AO_PT_DIR]
fin_trends_list = [INCOME_TRENDS_DIR, BS_TRENDS_DIR, CF_TRENDS_DIR]
## buffer
...
def make_training_data(growth_threshold=1.1, window=120) -> bool:
    """
    ...
    ## gete date
    fn = (
        glob.glob(TRAINING_BUFFER + "/*.csv")[0]
        .split("/")[-1]
        .split(".csv")[0]
        .split("T")[0]
        .split("-")
    ...
    ## start session
    spark = SparkSession.builder.appName("building-growth-training").getOrCreate()
    ...
    training_data["date"] = pd.to_datetime(training_data["date"]).apply(
        lambda r: r.date()
    )
    training_data["week_num"] = training_data["date"].apply(
        lambda r: r.isocalendar()[1]
    )
    ...
    if len(future_price) > 0:
        ...
        for col in ["future_close", "future_avg_close"]:
            future_price[col] = future_price.groupby("ticker")[col].fillna(
                method="ffill"
            )
        ## date and week num to join backwards
        future_price["date"] = future_price["future_date"] - dt.timedelta(90)
        future_price["week_num"] = future_price["date"].apply(
            lambda r: r.isocalendar()[1]
        )
        ## remove week duplicates for join
        future_price_distinct = (
            future_price.sort_values(["ticker", "week_num", "date"])
            .drop("date", 1)
            .drop_duplicates(["ticker", "week_num"])
        )
        ## join future data
        training_data = training_data.merge(
            future_price_distinct, how="left", on=["ticker", "week_num"]
        )
        ## clear space
        del future_price_distinct
        ## add growth bool
        training_data["growth"] = (
            training_data["future_close"] / training_data["avg_price"]
        ) >= growth_threshold
    ...
    date_ticker_explode_df = pd.concat(
        [explode_ticker(t, BASE_DF) for t in training_data["ticker"].unique()]
    ...
    for _dir in dir_sparse_list:
        ...
        tmp_df = (
            ...
            .filter(
                (
                    ## training window
                    ((F.col("date") >= date_min_extra) & (F.col("date") <= date))
                )
            )
            .orderBy(["ticker", "date"])
            .toPandas()
        )
        tmp_df["date"] = pd.to_datetime(tmp_df["date"]).apply(lambda r: r.date())
        ## fill missing
        full_tmp_df = date_ticker_explode_df.copy().merge(
            tmp_df, how="left", on=["date", "ticker"]
        )
        ## Scores lose magnitude as days since last scored
        ## moves farther in the past, up to 180 days.
        if _dir == AO_SCORE_DIR:
            full_tmp_df["weight"] = (180 - full_tmp_df["n_days_last_rating"]) / 180
            ## weight the change
            full_tmp_df["score_normalized_weighted"] = (
                full_tmp_df["weight"] * full_tmp_df["score_normalized"]
            )
            ## remove unneeded columns
            full_tmp_df = full_tmp_df.drop(["weight", "score_normalized"], 1)
        training_data = training_data.merge(
            full_tmp_df, how="left", on=["ticker", "date"]
        )
    ...
    for fin_type in fin_types:
        print(fin_type)
        data_location = sm_data_lake_dir + "/derived-measurements/{}-trends".format(
            fin_type
        )
        tmp_df = pd.DataFrame()
        ...
        for d in training_date_range + [date]:
            ...
            tmp_day_df["date"] = pd.to_datetime(tmp_day_df["date"]).apply(
                lambda r: r.date()
            )
            tmp_df = tmp_df.append(tmp_day_df).reset_index(drop=True)
        ...
        if (len([x for x in training_data.columns if "release_date" in x]) == 0) & (
            len([x for x in tmp_df.columns if "release_date" in x]) > 0
        ):
            tmp_df = tmp_df.drop(columns=["release_date"])
        training_data = training_data.merge(tmp_df, how="left", on=["ticker", "date"])
        del tmp_df
